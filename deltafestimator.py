...
class deltafestimator(hkl_model):
    ...
    def _build_graph(self, df, referencekey='FCALC', intensitykey='ipm2'):
        ...
        D = tf.constant(df[['GAMMAINDEX', 'D']].groupby('GAMMAINDEX').first().values.flatten(), dtype=tf.float32)
        ...
        sparsifier  = lp*tf.losses.mean_squared_error(tf.zeros(deltaF.shape), D*deltaF)
...
class deltafestimator_laplacian(hkl_model):
    ...
    def _build_graph(self, df, referencekey='FCALC', intensitykey='ipm2'):
        ...
        regularizer = rhop*tf.losses.mean_squared_error(ipm, Icryst)
        sparsifier  = lp*tf.linalg.norm(deltaF*D, 1)/tf.size(deltaF, out_type=tf.float32)
...
class deltafestimator_physical_gaussian(hkl_model):
    ...
    def _build_graph(self, df, referencekey='FCALC', intensitykey='ipm2'):
        ...
        r = runidx.max() + 1
        ipm  = tf.constant(np.float32(imagemetadata[intensitykey]))
        ipmx = np.float32(imagemetadata[intensitykey+'_xpos'])
        ipmx = tf.constant((ipmx - ipmx.mean())/ipmx.std())
        ipmy = np.float32(imagemetadata[intensitykey+'_ypos'])
        ipmy = tf.constant((ipmy - ipmy.mean())/ipmy.std())
        variables = tf.Variable(np.concatenate((
            np.ones(h, dtype=np.float32),
            -1.0*np.ones(r, dtype=np.float32),
             1.0*np.ones(r, dtype=np.float32),
            -1.0*np.ones(r, dtype=np.float32),
             1.0*np.ones(r, dtype=np.float32),
        ...
        deltaF = variables[:h]
        xmin = variables[h:h+r]
        xmax = variables[h+r:h+2*r]
        ymin = variables[h+2*r:h+3*r]
        ymax = variables[h+3*r:h+4*r]
        ipm_xscale = variables[-3]
        ipm_yscale = variables[-2]
        ipm_zp = variables[-1]
        gammas = deltaF/Foff + 1 #This is only here for backward compatibility. We will not use it directly
        DF = tf.gather(deltaF, gammaidx)
        Icryst = 0.25*(ipm + ipm_zp)*((
            tf.erf(tf.gather(xmax, runidx) - ipm_xscale*ipmx) - 
            tf.erf(tf.gather(xmin, runidx) - ipm_xscale*ipmx)
            ) * (
            tf.erf(tf.gather(ymax, runidx) - ipm_yscale*ipmy) - 
            tf.erf(tf.gather(ymin, runidx) - ipm_yscale*ipmy)
        ))
        Icryst = 0.01*tf.nn.softplus(100.*Icryst)
        ...
        sparsifier  = tf.losses.mean_squared_error(tf.zeros(deltaF.shape), D*deltaF)
        ...
        self.variables[intensitykey] = ipm
        self.variables['ipm-zero'] = ipm_zp
        self.variables['ipm-xscale'] = ipm_xscale
        self.variables['ipm-yscale'] = ipm_yscale
        self.logvars['Loss'] = loss
        self.logvars['Sparsifier'] = sparsifier
        self.logvars['Likelihood'] = likelihood
        self.miller_vars = ['DeltaF', 'SIGMA(DeltaF)', 'Hessian Diagonal', referencekey]

class deltafestimator_physical_gaussian2(hkl_model):
    def __init__(self, df):
        self._build_graph(df)
    ...
    def _build_graph(self, df, referencekey='FCALC', intensitykey='ipm2'):
        ...
        deltaF = variables[:h]
        xpos = variables[h:h+r]
        ypos = variables[h+r:h+2*r]
        ...
        gammas = deltaF/Foff + 1 #This is only here for backward compatibility. We will not use it directly
        DF = tf.gather(deltaF, gammaidx)
        xpos_ = tf.gather(xpos, runidx)
        ypos_ = tf.gather(ypos, runidx)
